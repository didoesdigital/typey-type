  ...
  if (
    shouldShowStroke(
      ...
      if (currentStroke) {
        ...
        let layoutTypeStyle = '';
        if (userSettings.stenoLayout === 'stenoLayoutKoreanModernCSteno') { layoutTypeStyle = ' heavy-type-face--korean'; }
        if (userSettings.stenoLayout === 'stenoLayoutJapaneseSteno') { layoutTypeStyle = ' type-face--japanese'; }

        const diagramWidth = (userSettings.diagramSize || 1) * 140;

        strokeTip = (
          <div className="stroke-tip" aria-live="polite" aria-atomic="true">
            <span className="visually-hidden" aria-hidden={userSettings.showStrokesAsDiagrams ? 'true' : 'false'}>Hint: </span>
            <div className="flex flex-wrap mr05">
              {userSettings.showStrokesAsDiagrams && strokes.map((strokeToDraw, index) =>
            ...
            {!userSettings.showStrokesAsDiagrams ?
              <div className={"db" + layoutTypeStyle}>
                <pre className="overflow-auto mw-408 text-small">
                  <span className="steno-stroke pa05 text-small" aria-label={[...currentStroke].join(" ").replace("-","dash")}>
