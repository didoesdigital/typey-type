...
class SingleLineMaterial extends Component {
  render() {
    ...
    const currentAndUpcomingPhrasesClasses = `dib current-and-upcoming-phrases${
      this.props.userSettings.blurMaterial ? " blur-words" : ""
    }`;
    const nextUpcomingMaterial =
      this.props.upcomingPhrases.length > 0
        ? this.props.upcomingPhrases[0]
        : "";
    ...
    const restUpcomingMaterial =
      this.props.upcomingPhrases.length > 1
        ? this.props.upcomingPhrases.slice(1).join(" ")
        : "";
          ...
          <div className="material mx-auto">
            <pre className="relative translateX-10px">
              <span className={completedPhrasesClasses}>
                {this.props.completedPhrases.join(" ")}&#8203;
              </span>
